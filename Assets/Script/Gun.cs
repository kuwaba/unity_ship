...
public class Gun : MonoBehaviour {
    ...
    void Update () {
        ...
        pivotObj = GameObject.Find("Gunpivot");
        transform.RotateAround(pivotObj.transform.position, new Vector3(0, 1, 0), diffy);

       Debug.Log("dy="+ transform.eulerAngles.x);
        //transform.Rotate(new Vector3(0f, dy, 0f));
        Debug.Log(transform.eulerAngles.x);
        if (315 < transform.eulerAngles.x || 1> transform.eulerAngles.x)
        {
        ...
        if (359 > transform.eulerAngles.x  && 270 < transform.eulerAngles.x)
        {
